...
async function getWeather() {
  try {
    const apiKey = "3d72b40ca76f72141ce490bb9f5e9925";
    ...
    document.getElementById("cal-weather-icon").textContent = icons[desc] ?? "🌡️";
    document.getElementById("cal-temp").textContent = temp + "°C";
    document.getElementById("cal-desc").textContent = data.weather[0].description;

  } catch (e) {
    document.getElementById("cal-temp").textContent = "N/A";
  }
}
getWeather();
...
async function fetchGitHubStats() {
  try {
    const res = await fetch("https://api.github.com/users/Aashi070407");
    const data = await res.json();
    ...
    // Fetch all repos to count stars + forks
    const reposRes = await fetch("https://api.github.com/users/Aashi070407/repos?per_page=100");
...
// ===== GITHUB CALENDAR =====
GitHubCalendar(".calendar", "Aashi070407", {
  responsive: true,
  tooltips: true,
  global_stats: false
});
